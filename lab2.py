...
3. Prediction
   ...
   'decision_tree.model' and be located in the same directory as lab2.py. The output is the predicted language (English, Spanish,
   or Polish).
"""
...
FEATURES_JSON_NAME = 'extracted_features.json'
NUM_FEATURES = 13
...
def main(args):
    if args[0] == "extract_features":
        ...
        print("Predicted language: {}".format(prediction.name))
...
def extract_mfcc():
    ...
    with open(FEATURES_JSON_NAME, 'w') as outfile:
...
def generate_model():
    ...
    ./decision_tree.model
    """
    with open(FEATURES_JSON_NAME, 'r') as examples_file:
...
def build_tree(data):
    ...
    for feature_num in range(NUM_FEATURES):
        feature_values = {}
        for row in data:
            feature_values[row[feature_num]] = 1
...
def dump_tree(tree, indent=''):
    if tree.results is not None:
        ...
        print(str(tree.feature_num) + ":" + str(tree.value) + "? ")
        print(indent + "T->", end=" ")
        dump_tree(tree.next_true, indent + "  ")
        print(indent + "F->", end=" ")
        dump_tree(tree.next_false, indent + "  ")
...
if __name__ == "__main__":
    """
    Dispatch based on command line args
    """
